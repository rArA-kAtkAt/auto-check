...
<style>
  ...
  input[type="search"]:focus {
    ...
    <?php 
    $pdo_statement = $pdo->prepare("SELECT *, sc.id as id, `sc`.status as 'statusclass' FROM tbl_student_class as sc LEFT JOIN tbl_class as c ON sc.class_id = c.id LEFT JOIN tbl_teacher as t ON c.teacher_id = t.id WHERE student_id = '".$student_id."'");
    $pdo_statement->execute();
    $result = $pdo_statement->fetchAll();
    ?>
	...
	if(!empty($result)) { 
      ...
      $condition = $row["statusclass"] == "Pending" ? " style='pointer-events: none;opacity: 0.5;'": "";
      $condition1 = $row["statusclass"] == 'Pending' ? '<div style="text-align:center">Waiting for approval</div>': '';

    echo '  <div class="card card1 p-3 mr-2 mt-2 card-design1" '.$condition.'>
    ...
    <div class="dropdown-menu" style="font-size:12px;">
      <a class="dropdown-item" href="../function.php?student-class-archive&i='.$row['id'].'">Archive</a>
      </div></div>
      <div onClick=\'location.href="tests.php?i='.$row['class_id'].'"\' type="button" class="text-center">
